...
export default class Device extends EventEmitter {
    ...
    clearSessionTime: number = 15 * 60; // in seconds
    clearSessionTimeout: ?number = null;
    clearSessionRest: number = 0;
    clearSessionRestInterval: ?number = null;
    ...
    _startClearSessionTimeout() {
        this.clearSessionTimeout = window.setTimeout(() => {
            ...
            this.clearSessionRest = 0;

            window.clearInterval(this.clearSessionRestInterval);
            this.clearSessionRestInterval = null;

            this.clearSessionTimeout = null;
        }, this.clearSessionTime);

        this.clearSessionRest = this.clearSessionTime;
        this.clearSessionRestInterval = window.setInterval(() => {
            this.clearSessionRest = this.clearSessionRest - 1000;
        }, 1000);
    }
    ...
    _stopClearSessionTimeout() {
        ...
        if (this.clearSessionRestInterval != null) {
            window.clearInterval(this.clearSessionRestInterval);
            this.clearSessionRestInterval = null;
        }
    }
