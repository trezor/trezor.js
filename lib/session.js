...
export default class Session extends EventEmitter {
    ...
    lastCallError: ?(Error & {transportError: boolean}) = null;
    ...
    deactivateEvents() {
        [
            ...
            this.wordEvent,
        ].forEach(ev => ev.removeAllListeners());
    }
    ...
    typedCall(type: string, resType: string, msg: Object = {}): Promise<DefaultMessageResponse> {
        return this.callHelper.typedCall(type, resType, msg).then(result => {
            this.lastCallError = null;
            return result;
        }, error => {
            this.lastCallError = error;
            throw error;
        });
    }
