...
function deriveOutputScript(
    ...
    scriptType: 'PAYTOADDRESS' | 'PAYTOSCRIPTHASH',
    externalNode: BitcoinJsHDNode,
    changeNode: BitcoinJsHDNode
): Buffer {
    const pkh: Buffer = typeof pathOrAddress === 'string'
                                ? BitcoinJsAddress.fromBase58(pathOrAddress).hash
...
function verifyBjsTx(
    ...
    if (inputs.length !== resTx.inputs.length) {
        throw new Error('Signed transaction has wrong length.');
    }
    if (outputs.length !== resTx.outputs.length) {
        throw new Error('Signed transaction has wrong length.');
    }
    outputs.map((output, i) => {
        if (output.value !== resTx.outputs[i].value) {
            throw new Error('Signed transaction has wrong output value.');
        }

        const addressOrPath_: ?(string | Array<number>) =
            output.address == null ? output.path : output.address;
        ...
        const scriptA: Buffer =
            deriveOutputScript(addressOrPath, output.scriptType, externalNode, changeNode);
        const scriptB: Buffer = resTx.outputs[i].script;
...
export default class Session extends EventEmitter {
    ...
    signTx(
        ...
        const coinName = (typeof coin === 'string') ? coinName : coin.coin_name;
    ...
    signBjsTx(
        ...
        const trezorInputs: Array<TransactionInput> = inputs.map(({input, path}) => {
            return {
        ...
        const trezorRefTxs: Array<RefTransaction> = refTxs.map((tx: BitcoinJsTransaction) => {
            return {
                ...
                version: tx.version,
                hash: tx.toHex(),
                inputs: tx.inputs.map((input: BitcoinJsInput) => {
                    return {
                        prev_index: input.index,
                        sequence: input.sequence,
                        prev_hash: input.hash,
                ...
                bin_outputs: tx.output.map((output: BitcoinJsOutput) => {
                    return {
                        amount: output.value,
        ...
        return this.signTx(
            ...
            verifyBjsTx(inputs, outputs, externalNode, changeNode, res);
            return res;
        });
    }
