...
export default class Session extends EventEmitter {
    ...
    initialize(): Promise<MessageResponse<trezortypes.Features>> {
        return this.typedCall('Initialize', 'Features');
    }

    getFeatures(): Promise<MessageResponse<trezortypes.Features>> {
    ...
    getAddress(address_n: Array<number>, coin: trezortypes.CoinType | string, show_display: ?boolean): Promise<MessageResponse<{
        address: string;
        path: Array<number>;
    }>> {
    ...
    getPublicKey(address_n: Array<number>): Promise<MessageResponse<trezortypes.PublicKey>> {
    ...
    resetDevice(settings: trezortypes.ResetDeviceSettings): Promise<DefaultMessageResponse> {
        return this.typedCall('ResetDevice', 'Success', settings);
    }

    loadDevice(
        settings: trezortypes.LoadDeviceSettings,
        network: trezortypes.CoinType | string | bitcoin.Network
    ): Promise<DefaultMessageResponse> {
        return this.typedCall('LoadDevice', 'Success', wrapLoadDevice(settings, coinNetwork(network)));
    }

    recoverDevice(settings: trezortypes.RecoverDeviceSettings): Promise<DefaultMessageResponse> {
    ...
    applySettings(settings: trezortypes.ApplySettings): Promise<DefaultMessageResponse> {
    ...
    updateFirmware(payload: string): Promise<DefaultMessageResponse> {
    ...
    signMessage(address_n: Array<number>, message: string, coin: trezortypes.CoinType | string): Promise<MessageResponse<trezortypes.MessageSignature>> {
    ...
    signIdentity(
        identity: trezortypes.Identity,
        challenge_hidden: string,
        challenge_visual: string
    ): Promise<MessageResponse<trezortypes.SignedIdentity>> {
    ...
    measureTx(
        ...
        outputs: Array<trezortypes.TransactionInput>,
        coin: string | trezortypes.CoinType
    ...
    verifyAddress(path: Array<number>, address: string, coin: string | trezortypes.CoinType): Promise<boolean> {
    ...
    getHDNode(path: Array<number>, network: trezortypes.CoinType | string | bitcoin.Network): Promise<bitcoin.HDNode> {
...
export function coinName(coin: trezortypes.CoinType | string): string {
...
export function coinNetwork(coin: trezortypes.CoinType | string | bitcoin.Network): bitcoin.Network {
    const r: any = coin;
