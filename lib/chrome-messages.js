            ...
            function (response) {
                if (response) {
                    if (response.type === "response") {
                        ...
                        console.log("[trezor] Error received", response);
                        reject(new Error(response.message));
                    } else {
                        console.log("[trezor] Unknown response type ", reponse.type);
                        reject("Unknown response type " + reponse.type)
                    }
                } else {
                    console.log("[trezor] Chrome runtime error", chrome.runtime.lastError);
