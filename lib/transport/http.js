...
export default class HttpTransport {
    ...
    enumerate(wait?: boolean, previous?: ?Array<DeviceDescriptor>): Promise<Array<DeviceDescriptor>> {
        const listenWithPrevious: boolean =
            !!(wait) &&
            (previous != null) &&
            (semvercmp(this.version, '1.1.3') >= 0);

        const enRes = listenWithPrevious
            ? this._request({
                method: 'POST',
                url: '/listen',
                body: previous,
        ...
        const checked = enRes.then(r => checkEnumerate(r));
        return checked;
    }
    ...
    release(sessionId: string|number): Promise<void> {
        return this._request(releaseOptions(sessionId)).then(() => {return;});
    }
