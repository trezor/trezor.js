...
function input2trezor(input: InputInfo): trezortypes.TransactionInput {
...
function output2trezor(output: OutputInfo, network: bitcoin.Network): trezortypes.TransactionOutput {
...
function signedTx2refTx(signedTx: MessageResponse<trezortypes.SignedTx>): bitcoin.Transaction {
    const res = bitcoin.Transaction.fromHex(signedTx.message.serialized.serialized_tx);
    return res;
}

function bjsTx2refTx(tx: bitcoin.Transaction): trezortypes.RefTransaction {
    return {
...
export function signBjsTx(
    ...
    const trezorInputs: Array<trezortypes.TransactionInput> = info.inputs.map(i => input2trezor(i));
    const trezorOutputs: Array<trezortypes.TransactionOutput> =
        info.outputs.map(o => output2trezor(o, network));
    const trezorRefTxs: Array<trezortypes.RefTransaction> = refTxs.map(tx => bjsTx2refTx(tx));
