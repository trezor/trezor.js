...
export function pubKey2bjsNode(
    key: MessageResponse<types.PublicKey>,
    network: bitcoin.Network
): bitcoin.HDNode {
...
export function getHDNode(
    ...
    return session.getPublicKey(path).then((resKey: MessageResponse<types.PublicKey>) => {
        const resNode: bitcoin.HDNode = pubKey2bjsNode(resKey, network);

        return session.getPublicKey(childPath).then((childKey: MessageResponse<types.PublicKey>) => {
