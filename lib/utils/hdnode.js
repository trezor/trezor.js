...
export function bjsNode2privNode(node: bitcoin.HDNode): types.HDPrivNode {
...
export function pubKey2bjsNode(
    key: types.MessageResponse<types.PublicKey>,
    network: bitcoin.Network
): bitcoin.HDNode {
    const keyNode: types.HDPubNode = key.message.node;
...
export function getHDNode(
    ...
    return session.getPublicKey(path).then((resKey: types.MessageResponse<types.PublicKey>) => {
        const resNode: bitcoin.HDNode = pubKey2bjsNode(resKey, network);

        return session.getPublicKey(childPath).then((childKey: types.MessageResponse<types.PublicKey>) => {
