...
export default class DeviceList extends EventEmitter {
    ...
    constructor(options: ?DeviceListOptions) {
        ...
        this.on('transport', (transport: Transport) => {
            ...
            // using setTimeout to emit 'stream' in next tick,
            // so people from outside can add
            // listener to 'stream' in listener to 'transport'
            setTimeout(() => this._initStream(transport), 0);
        });
