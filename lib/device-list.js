...
export default class DeviceList extends EventEmitter {
    ...
    _createDevice(transport: Transport, path: string, stream: DescriptorStream): Promise<Device|UnacquiredDevice> {
        const devRes: Promise<Device | UnacquiredDevice> = Device.fromPath(transport, path, this)
            .then((device: Device) => {
                ...
                if (error.message === WRONG_PREVIOUS_SESSION_ERROR_MESSAGE) {
                    const previousDevice = this.unacquiredDevices[path];
                    if (previousDevice == null) {
                        return this._createUnacquiredDevice(transport, path, stream);
                    }
                } else {
                    this.emit('error', error);
                    throw error;
    ...
    _createUnacquiredDevice(transport: Transport, path: string, stream: DescriptorStream): Promise<UnacquiredDevice> {
        const res = UnacquiredDevice.fromPath(transport, path, this)
        ...
        stream.on('update', (diff: DeviceDescriptorDiff) => {
            ...
            diff.connected.forEach((descriptor: DeviceDescriptor) => {
                ...
                if (descriptor.session == null) {
                    this.devicesPromises[path] = this._createDevice(transport, path, stream);
                } else {
                    this.devicesPromises[path] = this._createUnacquiredDevice(transport, path, stream);
                }
            });
