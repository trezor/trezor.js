    ...
    _createUnacquiredDevice(
        ...
        const res = UnacquiredDevice.fromDescriptor(transport, descriptor, this)
            .then((device: UnacquiredDevice) => {
                this.unacquiredDevices[pathStr] = device;
        ...
        stream.updateEvent.on((diff: DeviceDescriptorDiff) => {
            ...
            diff.connected.forEach((descriptor: DeviceDescriptor) => {
                ...
                if (descriptor.session == null) {
                    this.devicesPromises[path.toString()] = this._createDevice(transport, descriptor, stream);
                } else {
                    this.devicesPromises[path.toString()] = this._createUnacquiredDevice(transport, descriptor, stream);
                }
            });
            ...
            events.forEach(({d, e}) => {
                d.forEach((descriptor: DeviceDescriptor) => {
                    const path = descriptor.path;
                    const deviceP = this.devicesPromises[path.toString()];
                    deviceP.then((device) => {
            ...
            diff.disconnected.forEach((descriptor: DeviceDescriptor) => {
                const path = descriptor.path;
                const pathStr = path.toString();
                const deviceP = this.devicesPromises[pathStr];
                deviceP.then(device => {
                    if (device instanceof Device) {
