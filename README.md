...
The library is using new babelify with Babel 6 with several plugins. You will need to install the following packages:
     ...
     npm install --save-dev babel-preset-es2015 babel-plugin-add-module-exports babel-plugin-transform-class-properties babel-plugin-transform-flow-strip-types
...
| signMessage |  path:&nbsp;Array&lt;number&gt;<br>message:&nbsp;string<br>coin:&nbsp;string | Promise&lt;{<br>address:&nbsp; string,<br>signature:&nbsp;string<br>}&gt; | Signs a message. |
| verifyMessage | address:&nbsp;string<br>signature:&nbsp;string<br>message:&nbsp;string | Promise | Verifies a signed message. Rejects on wrong signature. |
...
| cipherKeyValue | path:&nbsp;Array&lt;number&gt;<br>key:&nbsp;string<br>value:&nbsp;Buffer<br>encrypt:&nbsp;boolean<br>ask\_on\_encrypt:&nbsp;[iv&nbsp;Buffer] | Promise&lt;<br>Response&lt;<br>{value:&nbsp;string<br>}&gt;&gt; | Encrypts a value on a given key.<br><br>Exact documentation of cipherKeyValue will be in [SLIP-0011](...) - meanwhile, you can check [TREZOR source code](https://github.com/trezor/trezor-mcu/blob/master/firmware/fsm.c#L451-L483). |
